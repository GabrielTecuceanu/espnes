...
static esp_err_t spi_bus_already_init(void) {
    return ESP_OK;
}
...
int sd_init(void) {
    ...
    host.init = spi_bus_already_init;

    sdspi_device_config_t dev_cfg = SDSPI_DEVICE_CONFIG_DEFAULT();
    dev_cfg.gpio_cs = PIN_CS_SD;
    dev_cfg.host_id = SPI2_HOST;

    esp_err_t ret = esp_vfs_fat_sdspi_mount(MOUNT_POINT, &host, &dev_cfg, &mount_cfg, &card);
    if (ret != ESP_OK) {
...
uint8_t* sd_load_rom(const char* path, size_t* out_size) {
    ...
    if (size < 16 || buf[0] != 0x4E || buf[1] != 0x45 || buf[2] != 0x53 || buf[3] != 0x1A) {
