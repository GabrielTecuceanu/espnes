...
void audio_init(int sample_rate)
{
    ...
    i2s_std_config_t std_cfg = {
        ...
        /* Stereo mode — duplicate mono samples to L+R in audio_write.
         * Avoids I2S mono DMA frame-size ambiguity on ESP32-S3. */
...
{
    if (!tx_chan || n_samples <= 0) return;
    if (n_samples > WRITE_BUF_FRAMES) n_samples = WRITE_BUF_FRAMES;

    /* Expand mono → stereo (L+R identical). Each frame = 4 bytes. */
