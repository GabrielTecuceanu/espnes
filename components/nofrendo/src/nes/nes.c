...
static nes6502_memwrite default_writehandler[] = {
    {0x0800, 0x1FFF, ram_write}, {0x2000, 0x3FFF, ppu_write},     {0x4000, 0x4013, apu_write},
    {0x4015, 0x4015, apu_write}, {0x4014, 0x4017, ppu_writehigh}, LAST_MEMORY_HANDLER};
...
static void mem_trash(uint8* buffer, int length) {
    ...
    for (i = 0; i < length; i++) buffer[i] = (uint8)rand();
}
